...
import { tableproducts } from "../data/TableProducts";
...
export default function CollectionTablePage() {
  ...
  const currentProducts = tableproducts?.slice(indexOfFirst, indexOfLast) || [];
  const totalPages = Math.ceil((tableproducts?.length || 0) / itemsPerPage);
  ...
  return (
    <>
      ...
      <div className="collection-container">
        <h2 className="product-title">TABLE WARE</h2>
        <div className="product-grid">
          {currentProducts.map((item) => {
            const to = item.Link || item.link || `/product/${item.id}`;
            return (
              <div key={item.id} className="product-card">
                <Link to="/productdetailpage3" aria-label={`${item.title} 상세 보기`}>
                  <img src={item.img} alt={item.title} />
                </Link>
                <div className="product-brand">{item.brand}</div>
                <div className="product-brand">{item.name}</div>
                <div className="product-name">
                  <Link to="/productdetailpage3">{item.title}</Link>
                </div>
